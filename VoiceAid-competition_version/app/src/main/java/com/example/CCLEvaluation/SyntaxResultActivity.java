...
public class SyntaxResultActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private void initData() throws Exception {
            ...
            if (jsonArray != null && jsonArray.length() > 0) {
                ...
                if (allQuestionsCompleted) {
                    for (int j = 0; j < jsonArray.length(); j++) {
                        ...
                        questionDetails.add(new QuestionDetail(num, question, result, audioPath, testStage, testLanguage));
                    }
                }
            }
        }
        ...
        // 生成评估建议
        generateEvaluationSuggestion(overallAccuracy, testPointCorrectCount, testPointTotalCount, totalCorrect, totalQuestions);
    ...
    private void generateEvaluationSuggestion(double overallAccuracy, Map<String, Integer> correctCount, Map<String, Integer> totalCount, int totalCorrect, int totalQuestions) {
        // 生成总体评估
        String overallEvaluation;
        if (totalQuestions >= 15) {
            // 题目总数达到15题，使用10/15的标准
            if (totalCorrect >= 10) {
                overallEvaluation = "从整体上来说，孩子的" + (format.equals("RG") ? "句法理解" : "句法表达") + "能力较好，基本达标，符合该年龄段孩子语言发育水平。";
            } else {
                overallEvaluation = "从整体上来说，孩子的" + (format.equals("RG") ? "句法理解" : "句法表达") + "能力还有待进一步发展，尚未达标。";
            }
        } else {
            // 题目总数不足15题，使用正确率标准
        ...
        for (String testPoint : totalCount.keySet()) {
            int correct = correctCount.getOrDefault(testPoint, 0);
            int total = totalCount.get(testPoint);
            double accuracy = (double) correct / total;

            if (accuracy == 0) {
                weaknessList.add(testPoint);
            } else if (Math.abs(accuracy - 1.0/3) < 0.01) {
        ...
        // 显示不稳定的能力
        LinearLayout unstableLayout = findViewById(R.id.unstable_list);

    ...
    private void initData() {
        try {
            ...
            for (int group = 1; group <= 6; group++) {
                ...
                int totalGroupQuestions = 10; // 每组10题
                
                // 尝试从SOCIAL+group的数组中加载数据
                JSONArray groupArray = evaluations.optJSONArray("SOCIAL" + group);
                if (groupArray != null && groupArray.length() > 0) {
                    for (int i = 0; i < groupArray.length(); i++) {
                        try {
                            JSONObject object = groupArray.optJSONObject(i);
                    ...
                    // 如果没有SOCIAL+group数组，尝试从SOCIAL数组中加载数据
                    for (int i = 0; i < 10; i++) {
                ...
                // 只有当整个组的题目都完成时才认为该组已完成
                if (completedCount == totalGroupQuestions) {
                    ...
                    for (int i = 0; i < groupArray.length(); i++) {
                        try {
                            ...
                            if (object != null && object.has("score") && !object.isNull("score")) {
                                ...
                                // 收集题目详情
                                int num = object.optInt("num", (group - 1) * 10 + i + 1);
                                String ability = object.optString("ability", "");
                                String focus = object.optString("focus", "");
                                String content = object.optString("content", "");
                                Integer score = object.optInt("score", 0);
                                totalScore += score;

                                // 收集需要重点关注和进一步发展的能力
                                if (score == 0 && !weaknessList.contains(focus) && !focus.isEmpty()) {
                                    weaknessList.add(focus);
                                } else if (score == 1 && !inProgressList.contains(focus) && !focus.isEmpty()) {

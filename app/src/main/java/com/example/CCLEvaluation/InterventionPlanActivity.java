...
public class InterventionPlanActivity extends AppCompatActivity {
    ...
    private void bindGuide(JSONObject guide) {
        ...
        if (etOverallSummary != null) etOverallSummary.setText(overallSummary);
        if (tvOverallSummary != null) tvOverallSummary.setText(safeOrPlaceholder(overallSummary));

        JSONArray mastered = guide.optJSONArray("mastered");
        if (etMastered != null) etMastered.setText(joinLines(mastered));
        if (tvMastered != null) tvMastered.setText(formatBulletList(mastered));

        if (etNotMasteredOverview != null) etNotMasteredOverview.setText(notMasteredOverview);
        if (tvNotMasteredOverview != null) tvNotMasteredOverview.setText(safeOrPlaceholder(notMasteredOverview));

        JSONArray focus = guide.optJSONArray("focus");
        if (etFocus != null) etFocus.setText(joinLines(focus));
        if (tvFocus != null) tvFocus.setText(formatBulletList(focus));

        JSONArray unstable = guide.optJSONArray("unstable");
        if (etUnstable != null) etUnstable.setText(joinLines(unstable));
        if (tvUnstable != null) tvUnstable.setText(formatBulletList(unstable));
        ...
        if (etSmartGoalText != null) etSmartGoalText.setText(smartText);
        if (tvSmartGoalText != null) tvSmartGoalText.setText(safeOrPlaceholder(smartText));

        if (etSmartGoalCycleWeeks != null) etSmartGoalCycleWeeks.setText(String.valueOf(cycleWeeks));
        if (etSmartGoalAccuracy != null) etSmartGoalAccuracy.setText(String.valueOf(accuracyThreshold));
        if (etSmartGoalSupport != null) etSmartGoalSupport.setText(joinLines(support));

        if (tvSmartMeta != null) {
            tvSmartMeta.setText("周期：" + cycleWeeks + " 周  ·  达标阈值：" + accuracyThreshold);
        }
        if (tvSmartGoalSupport != null) {
            tvSmartGoalSupport.setText(support == null || support.length() == 0 ? "" : formatBulletList(support));
        }

        JSONArray home = guide.optJSONArray("homeGuidance");
        if (etHomeGuidance != null) etHomeGuidance.setText(joinLines(home));
        if (tvHomeGuidance != null) tvHomeGuidance.setText(formatBulletList(home));

        JSONArray notes = guide.optJSONArray("notesForTherapist");
        if (etNotesForTherapist != null) etNotesForTherapist.setText(joinLines(notes));
        if (tvNotesForTherapist != null) tvNotesForTherapist.setText(formatBulletList(notes));
    }

...
//PN
    ...
    private static void addTreatmentPlanSection(Document document, Font titleFont, Font bodyFont, JSONObject plan) throws Exception {
        document.add(new Paragraph("干预指导/治疗方案", titleFont));
        document.add(new Paragraph(" ", bodyFont));

        JSONObject caseSummary = plan.optJSONObject("case_summary");
        if (caseSummary != null) {
            document.add(new Paragraph("个案摘要", titleFont));
            addKeyValue(document, "主诉", caseSummary.optString("chief_complaint", ""), bodyFont);
            addList(document, "关键发现", caseSummary.optJSONArray("key_findings"), bodyFont);
            addList(document, "疑似诊断", caseSummary.optJSONArray("suspected_diagnosis"), bodyFont);
            addList(document, "风险提示", caseSummary.optJSONArray("risk_flags"), bodyFont);
            document.add(new Paragraph(" ", bodyFont));
        }
        ...
        if (overall != null) {
            ...
            if (hasOverall) {
                document.add(new Paragraph("总体目标", titleFont));
                addList(document, "4 周内", overall.optJSONArray("within_4_weeks"), bodyFont);
                addList(document, "12 周内", overall.optJSONArray("within_12_weeks"), bodyFont);
                addList(document, "24 周内", overall.optJSONArray("within_24_weeks"), bodyFont);
                document.add(new Paragraph(" ", bodyFont));
            }
        }
        ...
        if (modulePlan != null) {
            ...
            if (hasAnyModule) {
                document.add(new Paragraph("模块计划", titleFont));
                addModulePlan(document, "构音/语音", modulePlan.optJSONObject("speech_sound"), bodyFont, true);
                addModulePlan(document, "语前", modulePlan.optJSONObject("prelinguistic"), bodyFont, false);
                addModulePlan(document, "词汇", modulePlan.optJSONObject("vocabulary"), bodyFont, false);
                addModulePlan(document, "句法", modulePlan.optJSONObject("syntax"), bodyFont, false);
                addModulePlan(document, "社交语用", modulePlan.optJSONObject("social_pragmatics"), bodyFont, false);
                document.add(new Paragraph(" ", bodyFont));
            }
        }
        ...
        if (schedule != null) {
            ...
            if (!sessions.isEmpty() || !minutes.isEmpty() || !review.isEmpty()) {
                document.add(new Paragraph("频次建议", titleFont));
                addKeyValue(document, "每周次数", sessions, bodyFont);
                addKeyValue(document, "每次时长", minutes, bodyFont);
                addKeyValue(document, "复评周期", review, bodyFont);
                document.add(new Paragraph(" ", bodyFont));
            }
        }

        JSONArray therapistNotes = plan.optJSONArray("notes_for_therapist");
        addList(document, "治疗师备注", therapistNotes, bodyFont);
        JSONArray parentNotes = plan.optJSONArray("notes_for_parents");
        addList(document, "家长建议", parentNotes, bodyFont);
    }
    ...
    public static void writeTreatmentPlanPdf(Context context, Uri uri, JSONObject childJson) throws Exception {
        ...
        if (plan == null) {
            throw new IllegalStateException("\u672a\u627e\u5230\u6cbb\u7597\u65b9\u6848");
        }
        ...
        try {
            ...
            document.open();
            Font simsun = FontFactory.getFont("assets/fonts/songsim.ttf", BaseFont.IDENTITY_H, BaseFont.EMBEDDED, 10);
            Font simsunBold = new com.itextpdf.text.Font(simsun.getBaseFont(), 12, Font.BOLD);
            addTreatmentPlanSection(document, simsunBold, simsun, plan);
        } finally {
            if (document.isOpen()) {
                document.close();
            }
    ...
    private static void addModulePlan(Document document, String title, JSONObject plan, Font bodyFont, boolean isSpeechSound) throws Exception {
        ...
        addList(document, "目标", plan.optJSONArray("targets"), bodyFont);
        if (isSpeechSound) {
            addList(document, "方法", plan.optJSONArray("methods"), bodyFont);
            addList(document, "活动", plan.optJSONArray("activities"), bodyFont);
            addList(document, "示例活动", plan.optJSONArray("sample_activities"), bodyFont);
        } else {
            addList(document, "活动", plan.optJSONArray("activities"), bodyFont);
            addList(document, "方法", plan.optJSONArray("methods"), bodyFont);
        }
        addList(document, "家庭练习", plan.optJSONArray("home_practice"), bodyFont);
        addList(document, "评估指标", plan.optJSONArray("metrics"), bodyFont);
        document.add(new Paragraph(" ", bodyFont));
    }
    ...
    private static void addKeyValue(Document document, String title, String value, Font bodyFont) throws Exception {
        ...
        document.add(new Paragraph(title + ": " + text, bodyFont));
    }

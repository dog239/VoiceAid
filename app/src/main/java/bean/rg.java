...
public class rg extends evaluation {
    ...
    public int handle(View[] views, int position) {
        ...
        if(position==0){
            ...
            // 处理第四组第十题的六个小问题
            if (num > 10 && num < 11) {
                // 显示小问题的题号，如10-1, 10-2等
                int subQuestionNum = (int)((num - 10) * 10);
                ((TextView)views[0]).setText("10-" + subQuestionNum);
            } else {
    ...
    public void test(View view, int position, List<Integer> ImageIdList,List<Integer []> ImageGroupIdList,
        ...
        nextButton.setOnClickListener(v -> {
            ...
            // 正确判断结果
            if(answer != null && right_option != null && answer.equals(right_option)){
    ...
    public void toJson(JSONObject evaluations) throws JSONException {
        ...
        if (groupNumber == 4 && num == 10) {
            ...
            for (int i = 0; i < subQuestions.length; i++) {
                ...
                if(time!=null){
                    // 为每个小问设置独立的结果
                    // 这里使用随机结果来模拟独立选择，实际应用中需要根据用户的实际选择来设置
                    // 暂时使用不同的结果来测试独立计分功能
                    boolean subResult = (i == 0) ? true : false; // 第一题正确，其他错误

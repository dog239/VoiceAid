 ...
 * 看图说故事（PST）
 */
public class pst extends evaluation {
    ...
    private final String colum3 = "答题时间";
    private final String colum4 = "录音";
    public pst(int num, Integer score, bean.audio audio, String time) {
        super(num);
        this.num = num;
        this.score = score;
        this.audio = audio;
    ...
    tvHint.setText("现在看这一张，每一张都是单独的故事，不同的人，不同的地方。 \n" +
            "这里(指着第一张)，然后(指着第二张)， \n" +
            "这里(指着第三张)，最后这里(指着第四张)， \n" +
            "你说说你看到了一个什么故事。");
    ...
    public void toJson(JSONObject evaluations) throws JSONException {
        ...
        evaluations.getJSONArray("PST").put(object);
    }

...
public class testcontext {
    ...
    private HashMap<String, Integer> groupCounts = new HashMap<>();//每个模块+场景+组的完成题目数量
    ...
    private String moduleKey = "";//当前评估模块
    ...
    public void incrementCount() {
        this.currentCount++;
        // 保存到分组计数中，使用模块+场景+组号作为键
        if (groupNumber != null) {
            String key = moduleKey + "_" + scene + "_" + groupNumber;
    ...
    public void setGroupNumber(Integer groupNumber) {
        // 保存当前组的计数，使用模块+场景+组号作为键
        if (this.groupNumber != null) {
            String key = moduleKey + "_" + scene + "_" + this.groupNumber;
            groupCounts.put(key, currentCount);
        }
        // 设置新的组号
        this.groupNumber = groupNumber;
        // 恢复新组的计数，如果没有则设置为0，使用模块+场景+组号作为键
        String newKey = moduleKey + "_" + scene + "_" + groupNumber;

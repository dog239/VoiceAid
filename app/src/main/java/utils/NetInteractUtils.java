...
public class NetInteractUtils {
    //定义服务器IP接口
    private final String url = "http://123.57.104.101/voiceaid";
        ...
        client.newCall(request).enqueue(new Callback() {
            ...
            public void onResponse(Call call, Response response) throws IOException {
                try {
                    ...
                    JSONObject jsonResponse = new JSONObject(responseData);
                    if (response.isSuccessful()) {//返回码为200
                        Object evaluationsObj = jsonResponse.get("evaluations");
                        String evaluations = evaluationsObj == null ? "[]" : evaluationsObj.toString();
        ...
        client.newCall(request).enqueue(new Callback() {
            ...
            public void onResponse(Call call, Response response) throws IOException {
                try {
                    ...
                    JSONObject jsonResponse = new JSONObject(responseData);
                    if (response.isSuccessful()) {//返回码为200
                        Object evaluationObj = jsonResponse.get("evaluation");
                        String evaluation = evaluationObj == null ? "{}" : evaluationObj.toString();
    ...
    private void registerCall(Request request, OkHttpClient client) {
        client.newCall(request).enqueue(new Callback() {
            @Override
                ...
                try {
                    ...
                    if (response.isSuccessful()) {
                        if (registerCallback != null) {
                            context.runOnUiThread(() -> registerCallback.onRegisterResult());
                        }
                    } else {
                        String message = jsonResponse.getString("message");
                        showToast(message);
